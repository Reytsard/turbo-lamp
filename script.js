...
const gameBoard = () => {
    ...
    const addGameBox = (player1, player2) => {
        ...
        boxButtons.forEach(button => {
            button.addEventListener('click',() => {
                changeBackground(playerTurn,button.value,gameArr);
                changePlayer();
            });
        });
    }
    const changeBackground = (player, indexValue, gameboard) =>{
            gameboard[indexValue] = player.playerNumber;
    }
    const changePlayer = () => {
        const gameStatus = gameArea.querySelector('.game-status');
    ...
    return {startIsPressed,showGreetings,removeInsideGameArea};
}

//main
const newGame = gameBoard();
const player1 = playerFactory("Rey", "X");
